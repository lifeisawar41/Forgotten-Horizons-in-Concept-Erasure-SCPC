...
class Imagenette(Dataset):
    def __init__(self, split, class_to_forget=None, transform=None):
        ...
        # self.dataset = load_dataset('/home/lc/Desktop/wza/ly/US-SD/prompts/imagenette.csv')[split]
        # self.dataset = Dataset.from_csv('/home/lc/Desktop/wza/ly/US-SD/prompts/imagenette.csv')
        # self.dataset = load_dataset('/home/lc/Desktop/wza/ly/US-SD/prompts/imagenette.csv')[split]
        self.dataset = load_dataset('parquet', data_files={'train': '/home/lc/Desktop/wza/ly/US-SD/imagenette/160px/train/0000.parquet'})[split] 
        self.class_to_idx = {
...
# class Imagenette(Dataset):
#     def __init__(self, split, class_to_forget=None, transform=None):
#         self.dataset = pd.read_csv('/home/lc/Desktop/wza/ly/US-SD/prompts/imagenette.csv')

#         self.class_to_idx = {
#             cls: i for i, cls in enumerate(self.dataset['label'].unique())  # 假设CSV的标签列为 'label'
#         }
#         self.file_to_class = {
#             row['image']: self.class_to_idx[row['label']]  # 假设CSV的列名为 'image' 和 'label'
...
#         label = self.class_to_idx[self.dataset.iloc[idx]['label']]
...
def setup_data(class_to_forget, batch_size, image_size, interpolation="bicubic"):
    ...
    print('descriptions of choosing class to erase:', descriptions )
...
def setup_remain_data(class_to_forget, batch_size, image_size, interpolation="bicubic",max_samples_per_class=None):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)
    ...
    descriptions = [f"an image of a {label}" for label in train_set.class_to_idx.keys()]
    filtered_data = [data for data in train_set if data[1] != class_to_forget]
    # Limit the number of samples per class for GA
    if max_samples_per_class is not None:
        filtered_data = filtered_data[:max_samples_per_class]
        print(f"Remain Filtered data length: {len(filtered_data)}")
    train_dl = DataLoader(filtered_data, batch_size=batch_size, shuffle=True)
    return train_dl, descriptions


def setup_forget_data(class_to_forget, batch_size, image_size, interpolation="bicubic",max_samples_per_class=None):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)
    ...
    filtered_data = [data for data in train_set if data[1] == class_to_forget]
    print(f"Forget  data length: {len(filtered_data)}")
    # Limit the number of samples per class for GA
    if max_samples_per_class is not None:
        filtered_data = filtered_data[:max_samples_per_class]
        print(f"Forget Filtered data length: {len(filtered_data)}")
    if len(filtered_data) == 0:
        print(f"No samples found for class {class_to_forget}")
